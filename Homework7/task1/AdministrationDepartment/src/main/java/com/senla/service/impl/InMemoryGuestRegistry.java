...
public class InMemoryGuestRegistry implements GuestRegistry {
    ...
    private List<Stay> staysInRoom(String numberRoom) {
        return stayIdsByRoom.getOrDefault(numberRoom, List.of()).stream()
                .map(staysById::get)
    ...
    @Override
    public List<GuestRegistry.GuestEntry> last3GuestsOfRoom(String numberRoom) {
        return staysInRoom(numberRoom).stream()
                .sorted(Comparator.comparing(Stay::end).reversed())
                .limit(3)
                .map(s -> new GuestRegistry.GuestEntry(
                        s.guest,
                        s.roomId,
                        s.start,
                        s.end,
