...
public class CheckInSortStatus implements SortStats {
    ...
    public void sortGuestsByAlphabetThenCheckout(GuestRegistry guests) {
        ...
        System.out.println("Постояльцы (имя ↑, дата выезда ↑):");
        for (var e : data) {
            System.out.println(" - " + e.guest + " — комната " + e.roomId +
    ...
    public void printRoomDetails(Rooms rooms, String roomId, GuestRegistry guests) {
        ...
        if (last3.isEmpty()) {
            System.out.println("  История: нет броней");
        } else {
            System.out.println("  Последние 3 проживания:");
            for (var e : last3) {
    ...
    public void printLast3GuestsOfRoom(String roomId, GuestRegistry guests) {
        var last3 = guests.last3GuestsOfRoom(roomId);
        System.out.println("Последние 3 постояльца комнаты " + roomId + ":");
        if (last3.isEmpty()) {
